...
def markAttendence(name):
    with open('attendence.csv', 'r+') as f:
        ...
        if name not in nameList:
            ...
            print("attendence is already marked")


# markAttendence("Name")

encodeListKnown = findEncodings(images)  #use hog algorithim to find encodings
# print(encodeListKnown)
# print(len(encodeListKnown))
print('Encoding Complete')

cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)  #if using  laptop's camera then use cv.VideoCapture(0)  and if using external camera use (1 )
